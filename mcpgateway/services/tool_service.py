...
class ToolService:
    ...
    async def invoke_tool(
        ...
        # ═══════════════════════════════════════════════════════════════════════════
        # PHASE 1: Check for X-Gateway-Id header for direct_proxy mode (no DB lookup)
        # ═══════════════════════════════════════════════════════════════════════════
        ...
        # If X-Gateway-Id header is present, check if gateway is in direct_proxy mode
        ...
        if gateway_id_from_header:
            ...
            if gateway and gateway.gateway_mode == "direct_proxy":
                ...
                logger.info(f"Direct proxy mode via X-Gateway-Id header: passing tool '{name}' directly to remote MCP server at {gateway.url}")
            elif gateway:
                ...
                logger.warning(f"Gateway {gateway_id_from_header} specified in X-Gateway-Id header not found")
                ...
                if tool_integration_type == "REST":
                    ...
                    async def connect_to_streamablehttp_server(server_url: str, headers: dict = headers):
                        ...
                        try:
                            ...
                            if use_pool and pool is not None:
                                # Pooled path: do NOT add per-request headers (they would be pinned)
                                # Ensure Accept header is set for MCP protocol compatibility
                                if "Accept" not in headers:
                                    headers["Accept"] = "application/json, text/event-stream"
                                
                                # Log request details for debugging
                                logger.info(f"[STREAMABLEHTTP-POOLED] Calling tool: {tool_name_original}")
                                logger.info(f"[STREAMABLEHTTP-POOLED] Endpoint: {server_url}")
                                logger.info(f"[STREAMABLEHTTP-POOLED] Headers: {headers}")
                                logger.info(f"[STREAMABLEHTTP-POOLED] Arguments: {arguments}")
                                ...
                                # Log request details for debugging
                                logger.info(f"[STREAMABLEHTTP] Calling tool: {tool_name_original}")
                                logger.info(f"[STREAMABLEHTTP] Endpoint: {server_url}")
                                logger.info(f"[STREAMABLEHTTP] Headers: {headers}")
                                logger.info(f"[STREAMABLEHTTP] Arguments: {arguments}")
                    ...
                    if is_direct_proxy:
                        ...
                        # If filtered_response contains error TextContent, mark as error
                        if isinstance(filtered_response, list) and len(filtered_response) > 0 and isinstance(filtered_response[0], TextContent):
                            is_err = True
